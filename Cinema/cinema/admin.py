...
from .models import Genre, Hall, Movie, Session, Ticket, Employee, News, Vacancy, FAQ, Contact, About, PromoCode, LogoModel, AdvModel, SponsorModel, HistoryEvent, Certificate
...
class PromoCodeAdmin(admin.ModelAdmin):
    ...
    start_date_only.short_description = "Дата начала"

    def end_date_only(self, obj):
        return obj.end_date.date()
    end_date_only.short_description = "Дата окончания"
...
class CompanyInfoAdmin(admin.ModelAdmin):
    list_display = ('content_preview', 'video_preview', 'company_name', 'inn', 'ogrn', 'address', 'bank')
    fields = ('content', 'video', 'company_name', 'inn', 'ogrn', 'address', 'bank')
...
class LogoAdmin(admin.ModelAdmin):
    list_display = ['logo']
    def has_add_permission(self, request):
        if LogoModel.objects.count() >= 1:
            return False
        return super().has_add_permission(request)
    
@admin.register(AdvModel)
class AdvModelAdmin(admin.ModelAdmin):
    list_display = ("id", "src")
...
class HistoryEventAdmin(admin.ModelAdmin):
    list_display = ("id", "about", "year", "text")
